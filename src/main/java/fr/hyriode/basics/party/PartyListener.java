...
public class PartyListener {
    ...
    public void onAccess(HyriPartyAccessEvent event) {
        ...
        for (UUID member : party.getMembers().keySet()) {
            ...
            if (player != null) {
                player.spigot().sendMessage(createMessage(builder -> builder.append((party.isPrivate() ? BasicsMessage.PARTY_ACCESS_PUBLIC_MESSAGE : BasicsMessage.PARTY_ACCESS_PRIVATE_MESSAGE).asString(player))));
            }
        }
    }
